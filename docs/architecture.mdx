...
description: "Design principles, intelligence tradecraft, algorithmic decisions, and implementation patterns behind Veritas Intel."
---

> **Looking for the system reference?** See [`ARCHITECTURE.md`](https://github.com/Rans-tech/VG-Intelligence-Terminal/blob/main/ARCHITECTURE.md) for deployment topology, directory layout, caching layers, CI/CD, and contributor how-to guides.
...
Veritas Intel is written in vanilla TypeScript — no frontend framework (React, Vue, Svelte, Angular) is used. This is a deliberate architectural decision, not an oversight.
...
All five dashboard variants (Veritas Intel, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor) serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:

| Hostname | Variant |
| --- | --- |
| `tech.intel.veritasglobal.co` | `tech` |
| `finance.intel.veritasglobal.co` | `finance` |
| `commodity.intel.veritasglobal.co` | `commodity` |
| `happy.intel.veritasglobal.co` | `happy` |
| `intel.veritasglobal.co` (default) | `full` |
...
Sentry captures unhandled exceptions and promise rejections in production, with environment-aware routing (production on `intel.veritasglobal.co`, preview on `*.vercel.app`, disabled on localhost and Tauri desktop).
