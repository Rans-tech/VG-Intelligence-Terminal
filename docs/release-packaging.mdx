...
description: "This guide provides reproducible local packaging steps for both desktop variants: full (Veritas Intel) and tech (Tech Monitor)."
---
...
- Full variant: `Veritas Intel` / `world-monitor`
- Tech variant: `Tech Monitor` / `tech-monitor`

Distinct names are configured in Tauri:

- `src-tauri/tauri.conf.json` → `Veritas Intel` / `world-monitor`
