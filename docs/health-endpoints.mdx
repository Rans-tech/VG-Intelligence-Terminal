...
```bash
# Full health check
curl -s https://intel.veritasglobal.co/api/health | jq .

# Compact (problems only)
curl -s "https://intel.veritasglobal.co/api/health?compact=1" | jq .

# UptimeRobot / monitoring: check HTTP status code
curl -o /dev/null -s -w "%{http_code}" https://intel.veritasglobal.co/api/health
...
```bash
curl -s https://intel.veritasglobal.co/api/seed-health \
...
STATUS=$(curl -s "https://intel.veritasglobal.co/api/health?compact=1")
