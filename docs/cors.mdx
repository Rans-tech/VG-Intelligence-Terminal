...
| Pattern | Matches |
| --- | --- |
| `(*.)?intel.veritasglobal.co` | Production + subdomains (`tech.`, `finance.`, etc.) |
...
- `Content-Type`
- `Authorization`
- `X-Veritas-Key` (API key for desktop/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
