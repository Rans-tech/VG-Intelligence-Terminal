...
async function warmPingChokepoints() {
  ...
  try {
    const resp = await fetch(`${baseUrl}/api/supply-chain/v1/get-chokepoint-status`, {
      headers: { 'User-Agent': CHROME_UA, Origin: 'https://intel.veritasglobal.co' },
    ...
    try {
      const resp = await fetch(provider.apiUrl, {
        ...
        headers: {
          ...
          ...(provider.name === 'openrouter' ? { 'HTTP-Referer': 'https://intel.veritasglobal.co', 'X-Title': 'Veritas Intel' } : {}),
        },
        body: JSON.stringify({
