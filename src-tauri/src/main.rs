...
fn open_live_channels_window(app: &AppHandle, base_url: Option<String>) -> Result<(), String> {
    ...
    .title("Channel management - Veritas Intel")
    .title_bar_style(tauri::TitleBarStyle::Overlay)
...
fn build_app_menu(handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let about_metadata = AboutMetadata {
        name: Some("Veritas Global Intelligence Terminal".into()),
        version: Some(env!("CARGO_PKG_VERSION").into()),
        copyright: Some("\u{00a9} 2025 Veritas Global, LLC".into()),
        website: Some("https://intel.veritasglobal.co".into()),
        website_label: Some("intel.veritasglobal.co".into()),
        ..Default::default()
    };
    let about_item =
        PredefinedMenuItem::about(handle, Some("About Veritas Intel"), Some(about_metadata))?;
...
fn handle_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        MENU_HELP_GITHUB_ID => {
            let _ = open_in_shell("https://github.com/Rans-tech/VG-Intelligence-Terminal");
        }
        #[cfg(feature = "devtools")]
        MENU_HELP_DEVTOOLS_ID => {
