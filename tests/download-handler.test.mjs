...
test('matches full variant for Veritas-Intel AppImage asset names', async () => {
  ...
  try {
    const response = await handler(
      new Request('https://intel.veritasglobal.co/api/download?platform=linux-appimage&variant=full')
    );
...
test('matches tech variant for dashed Tech-Monitor AppImage asset names', async () => {
  ...
  try {
    const response = await handler(
      new Request('https://intel.veritasglobal.co/api/download?platform=linux-appimage&variant=tech')
    );
    assert.equal(response.status, 302);
    assert.equal(
      response.headers.get('location'),
...
test('falls back to release page when requested variant has no matching asset', async () => {
  ...
  try {
    const response = await handler(
      new Request('https://intel.veritasglobal.co/api/download?platform=linux-appimage&variant=finance')
    );
    assert.equal(response.status, 302);
