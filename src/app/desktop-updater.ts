...
export class DesktopUpdater implements AppModule {
  ...
  private async checkForUpdate(): Promise<void> {
    try {
      const res = await fetch('https://api.intel.veritasglobal.co/api/version', {
      ...
      const releaseUrl = typeof data.url === 'string' && data.url
        ? data.url
        : 'https://github.com/Rans-tech/VG-Intelligence-Terminal/releases/latest';
  ...
  private async resolveUpdateDownloadUrl(releaseUrl: string): Promise<string> {
    try {
      ...
      if (platform) {
        ...
        return `https://api.intel.veritasglobal.co/api/download?platform=${platform}&variant=${variant}`;
      }
    } catch {
