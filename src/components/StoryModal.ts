...
function downloadStory(): void {
  ...
  a.download = `veritas-intel-${currentData?.countryCode.toLowerCase() || 'story'}-${Date.now()}.png`;
  a.click();
...
async function shareWhatsApp(data: StoryData): Promise<void> {
  ...
  const file = new File([currentBlob], `${data.countryCode.toLowerCase()}-veritas-intel.png`, { type: 'image/png' });
