...
export class Panel {
  ...
  constructor(options: PanelOptions) {
    ...
    if (isDesktopRuntime() && options.premium === 'enhanced' && !getSecretState('VERITAS_API_KEY').present) {
  ...
  public showLocked(features: string[] = []): void {
    ...
    if (isDesktopRuntime()) {
      ctaBtn.addEventListener('click', () => void invokeTauri<void>('open_url', { url: 'https://intel.veritasglobal.co/pro' }).catch(() => window.open('https://intel.veritasglobal.co/pro', '_blank')));
    } else {
      ctaBtn.addEventListener('click', () => window.open('https://intel.veritasglobal.co/pro', '_blank'));
    }
