...
const BROWSER_ORIGIN_PATTERNS = [
  /^https:\/\/(.*\.)?veritasglobal\.co$/,
  /^https:\/\/vg-intelligence-[a-z0-9-]+\.vercel\.app$/,
...
export function validateApiKey(req, options = {}) {
  ...
  const key = req.headers.get('X-Veritas-Key');
  ...
  if (isDesktopOrigin(origin)) {
    ...
    const validKeys = (process.env.VERITAS_VALID_KEYS || '').split(',').filter(Boolean);
    if (!validKeys.includes(key)) return { valid: false, required: true, error: 'Invalid API key' };
    return { valid: true, required: true };
  }

  // Trusted browser origin (veritasglobal.co, Vercel previews, localhost dev) — no key needed
